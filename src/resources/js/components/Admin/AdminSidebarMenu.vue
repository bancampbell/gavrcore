<template>
    <aside :class="[
        'admin-sidebar fixed lg:relative z-30 w-64 h-screen overflow-y-auto transition-transform duration-300 ease-in-out',
        isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
    ]">
        <div class="py-4">
            <div class="pl-3 mb-4">
                <div class="bg-gray-50 rounded-lg p-3 border border-gray-200 version-block">
                    <div class="text-xs font-semibold text-gray-500 uppercase tracking-wider version-label">Менеджер меню</div>
                    <div class="text-sm text-gray-700 version-number">Управление навигацией</div>
                </div>
            </div>

            <nav class="space-y-2">
                <SidebarLink href="/admin/menu" icon="menu" :active="isActive('/admin/menu')">
                    Менеджер меню
                </SidebarLink>
                <SidebarLink :href="allMenuLink" icon="list" :active="isActive(allMenuLink) && !isActive('/admin/menu')" :disabled="!currentMenuTypeId">
                    Все меню
                </SidebarLink>
            </nav>

            <div v-if="currentMenuTypeTitle && isMenuItemsPage" class="mt-6 pl-3 pt-4 border-t border-gray-200">
