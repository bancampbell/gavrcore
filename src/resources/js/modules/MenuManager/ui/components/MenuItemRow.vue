<template>
    <div class="grid gap-4 px-4 py-3 text-sm hover:bg-gray-50 border-b border-gray-100"
        ...
        <div class="flex items-center justify-center">
            <input type="checkbox" :checked="selected" @change="$emit('toggle-select', item.id)" class="rounded border-gray-300" />
        </div>
        ...
        <div class="flex items-start" :style="{ paddingLeft: `${level * 20}px` }">
            <div>
                <button @click="$emit('edit', item.id)" class="font-medium text-[#3071a9] hover:underline text-left">{{ item.title }}</button>
                <div class="text-xs text-gray-400 mt-0.5">{{ getLinkTypeLabel(item.link_type) }}: {{ getLinkValueDisplay(item.link_type, item.link_value) }}</div>
            </div>
        </div>
        <div class="flex items-center">
            <span :class="getMenuColorClass(item.menu_type?.title)">{{ item.menu_type?.title || '—' }}</span>
        </div>
        <div class="flex items-center justify-center">
            <span @click="$emit('toggle-status', item.id, !item.status)"
                :class="['px-2 py-1 text-xs rounded-full font-medium cursor-pointer transition whitespace-nowrap', item.status ? 'bg-green-100 text-green-800 hover:bg-green-200' : 'bg-red-100 text-red-700 hover:bg-red-200']">
...
defineProps<{ item: MenuItem & { level?: number; menu_type?: { title: string } }; level: number; selected: boolean; index?: number }>();
defineEmits<{ (e: 'toggle-select', id: number): void; (e: 'edit', id: number): void; (e: 'toggle-status', id: number, status: boolean): void }>();

const getLinkTypeLabel = (type: string) => {
    const labels: Record<string, string> = { url: 'URL', material: 'Материал', separator: 'Разделитель', heading: 'Заголовок', external: 'Внешний URL' };
...
const getMenuColorClass = (menuTitle?: string) => {
    ...
    if (menuTitle && colors[menuTitle]) return colors[menuTitle];
