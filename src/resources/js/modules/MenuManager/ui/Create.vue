<template>
    <AdminLayout :user="user">
                        ...
                        <div class="flex-1">
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <select v-model="form.link_type" class="admin-form-select w-full max-w-md">
                            ...
                            <div>
                                ...
                                <div class="space-y-2">
                                    <label class="flex items-center gap-2">
                                        <input type="radio" value="first" v-model="positionType" class="rounded border-gray-300" />
                                        <span class="text-sm text-slate-700">Первым (в начало списка)</span>
                                    </label>
                                    <label class="flex items-center gap-2">
                                        <input type="radio" value="after" v-model="positionType" class="rounded border-gray-300" />
                                ...
                                <select v-if="positionType === 'after'" v-model="form.after_id" class="admin-form-select w-full mt-2">
                                    ...
                                    <option v-for="item in orderOptions" :key="item.id" :value="item.id">{{ '—'.repeat(item.level || 0) }} {{ item.title }}</option>
                                </select>
                            </div>
                            <div>
                                ...
                                <select v-model="form.parent_id" class="admin-form-select w-full">
                                    ...
                                    <option v-for="item in parentOptions" :key="item.id" :value="item.id">{{ '—'.repeat(item.level || 0) }} {{ item.title }}</option>
                                </select>
                            </div>
                            <div>
        ...
        <MaterialSelectorModal :show="showMaterialModal" @close="showMaterialModal = false" @select="selectMaterial" />
    </AdminLayout>
</template>
...
const props = defineProps<{ user: any; title?: string; menuTypeId: number }>();
...
const onMenuTypeChange = () => { loadParentOptions(); loadOrderOptions(); };
...
const loadParentOptions = async () => {
    ...
    try {
        const r = await menuItemsApi.getTree(form.value.menu_type_id);
        parentOptions.value = flatten(r.data || []);
    } catch (e) {
        console.error(e);
        parentOptions.value = [];
    }
...
const flatten = (items: MenuItem[], level = 0): (MenuItem & { level: number })[] => {
    ...
    for (const item of items) {
        ...
        if (item.children?.length) result = [...result, ...flatten(item.children, level + 1)];
    }
    return result;
};
...
const updateAlias = () => {
    ...
    const ruMap: Record<string, string> = {
        'а':'a','б':'b','в':'v','г':'g','д':'d','е':'e','ё':'e','ж':'zh','з':'z','и':'i','й':'y','к':'k','л':'l','м':'m','н':'n','о':'o','п':'p','р':'r','с':'s','т':'t','у':'u','ф':'f','х':'h','ц':'c','ч':'ch','ш':'sh','щ':'sh','ъ':'','ы':'y','ь':'','э':'e','ю':'yu','я':'ya'
    };
...
const save = async () => {
    ...
    loading.value = true;
    try {
        const response = await menuItemsApi.create(form.value.menu_type_id, prepareSubmitData());
        showNotification('Пункт меню сохранён', 'success');
        router.visit(`/admin/menu/items/${response.data.data.id}/edit`);
    } catch (error: any) {
