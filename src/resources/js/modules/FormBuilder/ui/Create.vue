        ...
        <div class="admin-page-card w-full p-6">
          <div class="max-w-2xl space-y-6">
            ...
            <div>
              ...
              <p class="text-xs text-slate-400 mt-1">Укажите один или несколько email-адресов через запятую. Письма будут отправляться при каждом новом обращении.</p>
            </div>
            <div>
...
const form = ref({ title: '', alias: '', description: '', status: true, notificationEmailsInput: '' });
...
const save = async () => {
  if (!form.value.title.trim()) { showNotification('Введите название', 'error'); return; }
  loading.value = true;
  try {
    await axios.post('/admin/forms', buildPayload());
    showNotification('Форма создана', 'success');
    form.value = { title: '', alias: '', description: '', status: true, notificationEmailsInput: '' };
    isAliasManuallyEdited.value = false;
  } catch (error: any) { showNotification(error.response?.data?.message || 'Ошибка при сохранении', 'error'); }
...
const saveAndClose = async () => {
  ...
  } catch (error: any) { showNotification(error.response?.data?.message || 'Ошибка при сохранении', 'error'); loading.value = false; }
};
